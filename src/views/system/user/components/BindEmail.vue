...
const getCode = async (type:number)=> {
  ...
  if(data.code === 200){
    ElMessage.success('验证码已经发送到您的邮箱，此验证码用于更换邮箱绑定，请勿将验证码告知他人！')
  }
...
<template>
  <div class="set">
    <h4>绑定邮箱</h4>
    <p><span>已绑定邮箱：</span><span>{{userEmail}}</span>
    </p>
          ...
          <el-col :span="8" v-show="showNewEmail && !showGetCode">
            <el-form-item prop="code2" style="width: 100%;">
              <el-input v-model="toBind.code2" placeholder="请输入新邮箱验证码">
                <template #append>
                  <input type="button" :plain="true" @click="getCode(2)" :disabled="!show" style="width: 100%;height: 100%;
border: 0px;background: none;color: #ababab;cursor: pointer;" :value="codeText">
                </template>
              </el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="24" v-show="!showGetCode && !showNewEmail">
            <el-button plain color="#2fa7b9" @click="showGetCode=true" style="margin-left: 0px;">
              更换绑定邮箱
            </el-button>
          </el-col>
